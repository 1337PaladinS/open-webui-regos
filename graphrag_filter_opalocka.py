...
class Filter:
    ...
    Neo4j for relevant Opa-Locka Code of Ordinances regulatory sections, and prepends them
    ...
    class Valves(BaseModel):
        ...
        guardrail_jurisdiction_enabled: bool = Field(
            default=True,
            description="Enable jurisdiction mismatch detection. Flags queries that explicitly reference locations outside City of Opa-Locka.",
        )
        guardrail_jurisdiction_allowlist: str = Field(
            default="opa-locka,opa locka,opalocka,miami-dade,dade county,south florida,florida",
        ...
        threshold_check_enabled: bool = Field(
            default=True,
            description="Automatically detect numeric measurements in queries and evaluate against Opa-Locka Code of Ordinances thresholds. Works with any model — no tool-calling support needed.",
        )
...
- Start your answer with: "**[GraphRAG + KB]**" if you used both graph and KB context, or "**[GraphRAG]**" if only graph context was available."""
    ...
    def _build_disclaimer(self) -> str:
        ...
        if band == "HIGH":
            ...
            return (
                ...
                f"(composite confidence: {pct}%). "
                f"Please consult the the official City of Opa-Locka Code of Ordinances directly "
        ...
        for det in detections:
            ...
            if not matches:
                all_determinations.append({
                    ...
                    "user_value": value,
                    "user_unit": unit,
                    "message": f"No regulatory threshold found for '{param}' in Opa-Locka Code of Ordinances.",
                })
                continue
    ...
    def _build_threshold_context(self, determinations: list[dict]) -> str:
        ...
        lines = [
            ...
            "against Opa-Locka Code of Ordinances regulatory thresholds. These are EXACT determinations",
    ...
    def _build_compliance_badge(self, determinations: list[dict]) -> str:
        ...
        lines.append("*This determination was computed programmatically from Opa-Locka Code of Ordinances regulatory thresholds and recorded with a SHA-256 evidence hash for audit verification.*")
    ...
    # Out-of-scope intent prototypes — queries clearly NOT in Opa-Locka's code
    _OOS_PROTOTYPES = [
    ...
    # In-scope intent prototypes — queries that ARE in Opa-Locka's code
    _IS_PROTOTYPES = [
        "What are the business tax receipt requirements in Opa-Locka?",
        "City commission meeting procedures and rules of order",
        "Building permit requirements under the Florida Building Code",
        "Code of ethics and conflict of interest rules for city officials",
        "Zoning and land development regulations in Opa-Locka",
        "Public works and utilities department responsibilities",
        "Fire prevention and emergency services requirements",
        "Nuisance abatement and code enforcement procedures",
        "Parks and recreation facility use regulations",
        "Alcoholic beverage licensing requirements in Opa-Locka",
    ]
    ...
    def _check_zero_retrieval(self, entities: list, sections: list) -> tuple[bool, str]:
        ...
        Returns (triggered: bool, reason: str).
        This indicates the query has no connection to any Opa-Locka Code of Ordinances content.
        """
        if len(entities) == 0 and len(sections) == 0:
            return True, "No regulatory entities or sections found — query has no connection to Opa-Locka Code of Ordinances content"
        return False, ""
    ...
    def _check_jurisdiction_mismatch(self, query: str) -> tuple[bool, str]:
        ...
        if matched_locations:
            ...
            return True, (
                f"Query references a jurisdiction outside City of Opa-Locka: {locations_str}. "
                f"Opa-Locka Code of Ordinances applies only to City of Opa-Locka, Florida."
            )
    ...
    _SYSTEM_PROMPT_DEFENSE = (
        ...
        "Priority 1 (HIGHEST): Your core identity as RegOS Opa-Locka Municipal Code Assistant.\n"
    ...
    def _build_output_validation_notice(self, validation: dict) -> str:
        ...
        if grounding:
            ...
            parts.append(
                ...
                f"against the full Opa-Locka Code of Ordinances text."
            )
    ...
    def _build_guardrail_notice(self) -> str:
        ...
        if self._guardrail_type == "out_of_scope":
            ...
            body = (
                ...
                "RegOS is designed to answer questions about the City of Opa-Locka Code of Ordinances and Land Development Regulations"
                "(Opa-Locka Municipal Code) and does not cover the topic referenced "
                "in your query."
            )
            next_steps = (
                "If your question relates to Opa-Locka Code of Ordinances, try rephrasing with specific "
                "regulatory terms (e.g., business tax receipts, zoning, building permits, code enforcement). "
                "For topics outside Opa-Locka Code of Ordinances, consult the relevant regulatory authority directly."
            )
            ...
            body = (
                ...
                "City of Opa-Locka, Florida. Opa-Locka Code of Ordinances of the City of Opa-Locka Code "
                "applies exclusively to facilities and operations within City of Opa-Locka "
            ...
            next_steps = (
                ...
                "does involve a City of Opa-Locka facility, try rephrasing without the "
                "external location reference."
            )
        elif self._guardrail_type == "zero_retrieval":
            title = "No Regulatory Context Found"
            body = (
                "RegOS could not locate any Opa-Locka Code of Ordinances sections relevant to this query. "
                "This typically means the topic falls outside the scope of Opa-Locka Code of Ordinances, "
                "or the question may need more specific regulatory terminology."
            )
            next_steps = (
                "Try rephrasing your question with specific Opa-Locka Code of Ordinances section references "
            ...
            next_steps = (
                ...
                "or 'system override'). RegOS is designed to answer Opa-Locka Code of Ordinances compliance questions."
            )
            ...
            body = (
                ...
                if self.valves.neo4j_failover_message
                    ...
                    "Opa-Locka Code knowledge graph database. Do not rely on this response for "
            ...
            next_steps = (
                "Try rephrasing your question to focus on Opa-Locka Code of Ordinances compliance requirements."
            )
...
and cross-referenced sections in the Opa-Locka municipal code knowledge graph.
            ...
            for i in range(len(messages) - 1, -1, -1):
                if messages[i].get("role") == "user":
                    messages[i]["content"] = (
                        ...
                        "assistant for City of Opa-Locka Code of Ordinances. I can help with "
        ...
        if self.valves.neo4j_failover_enabled:
            ...
            if not neo4j_ok:
                ...
                failover_system = (
                    ...
                    "You are responding WITHOUT any Opa-Locka Code of Ordinances regulatory context. "
