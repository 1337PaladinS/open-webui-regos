...
class Filter:
    ...
    Neo4j for relevant Village of Pinecrest Code of Ordinances regulatory sections, and prepends them
    ...
    class Valves(BaseModel):
        # Neo4j connection
        neo4j_uri: str = Field(
            default="neo4j+s://d1524b84.databases.neo4j.io",
        ...
        guardrail_jurisdiction_enabled: bool = Field(
            default=True,
            description="Enable jurisdiction mismatch detection. Flags queries that explicitly reference locations outside Village of Pinecrest.",
        )
        guardrail_jurisdiction_allowlist: str = Field(
            default="pinecrest,miami-dade,dade county,south florida,florida",
        ...
        threshold_check_enabled: bool = Field(
            default=True,
            description="Automatically detect numeric measurements in queries and evaluate against Village of Pinecrest Code of Ordinances thresholds. Works with any model — no tool-calling support needed.",
        )
...
- Start your answer with: "**[GraphRAG + KB]**" if you used both graph and KB context, or "**[GraphRAG]**" if only graph context was available."""
    ...
    def _build_disclaimer(self) -> str:
        ...
        if band == "HIGH":
            ...
            return (
                ...
                f"(composite confidence: {pct}%). "
                f"Please consult the the official Village of Pinecrest Code of Ordinances directly "
        ...
        for det in detections:
            ...
            if not matches:
                all_determinations.append({
                    ...
                    "user_value": value,
                    "user_unit": unit,
                    "message": f"No regulatory threshold found for '{param}' in Village of Pinecrest Code of Ordinances.",
                })
                continue
    ...
    def _build_threshold_context(self, determinations: list[dict]) -> str:
        ...
        lines = [
            ...
            "against Village of Pinecrest Code of Ordinances regulatory thresholds. These are EXACT determinations",
    ...
    def _build_compliance_badge(self, determinations: list[dict]) -> str:
        ...
        lines.append("*This determination was computed programmatically from Village of Pinecrest Code of Ordinances regulatory thresholds and recorded with a SHA-256 evidence hash for audit verification.*")
    ...
    # Out-of-scope intent prototypes — queries clearly NOT in Pinecrest's code
    _OOS_PROTOTYPES = [
    ...
    # In-scope intent prototypes — queries that ARE in Pinecrest's code
    _IS_PROTOTYPES = [
        "What are the building permit requirements in Pinecrest?",
        "Village council meeting procedures and rules of order",
        "Zoning and land use regulations in Pinecrest",
        "Tree preservation and canopy requirements in Pinecrest",
        "Code enforcement procedures and violation penalties",
        "Stormwater management regulations in Pinecrest",
        "Home occupation and business license requirements",
        "Historic preservation and architectural review standards",
        "Noise and nuisance abatement ordinances in Pinecrest",
        "Special events and public assembly permit requirements",
    ]
    ...
    def _check_zero_retrieval(self, entities: list, sections: list) -> tuple[bool, str]:
        ...
        Returns (triggered: bool, reason: str).
        This indicates the query has no connection to any Village of Pinecrest Code of Ordinances content.
        """
        if len(entities) == 0 and len(sections) == 0:
            return True, "No regulatory entities or sections found — query has no connection to Village of Pinecrest Code of Ordinances content"
        return False, ""
    ...
    def _check_jurisdiction_mismatch(self, query: str) -> tuple[bool, str]:
        ...
        if matched_locations:
            ...
            return True, (
                f"Query references a jurisdiction outside Village of Pinecrest: {locations_str}. "
                f"Village of Pinecrest Code of Ordinances applies only to Village of Pinecrest, Florida."
            )
    ...
    _SYSTEM_PROMPT_DEFENSE = (
        ...
        "Priority 1 (HIGHEST): Your core identity as RegOS Pinecrest Municipal Code Assistant.\n"
    ...
    def _build_output_validation_notice(self, validation: dict) -> str:
        ...
        if grounding:
            ...
            parts.append(
                ...
                f"against the full Village of Pinecrest Code of Ordinances text."
            )
    ...
    def _build_guardrail_notice(self) -> str:
        ...
        if self._guardrail_type == "out_of_scope":
            ...
            body = (
                ...
                "RegOS is designed to answer questions about the Village of Pinecrest Code of Ordinances and Land Development Regulations"
                "(Pinecrest Municipal Code) and does not cover the topic referenced "
                "in your query."
            )
            next_steps = (
                "If your question relates to Village of Pinecrest Code of Ordinances, try rephrasing with specific "
                "regulatory terms (e.g., business tax receipts, zoning, building permits, code enforcement). "
                "For topics outside Village of Pinecrest Code of Ordinances, consult the relevant regulatory authority directly."
            )
            ...
            body = (
                ...
                "Village of Pinecrest, Florida. Village of Pinecrest Code of Ordinances of the Village of Pinecrest Code "
                "applies exclusively to facilities and operations within Village of Pinecrest "
            ...
            next_steps = (
                ...
                "does involve a Village of Pinecrest facility, try rephrasing without the "
                "external location reference."
            )
        elif self._guardrail_type == "zero_retrieval":
            title = "No Regulatory Context Found"
            body = (
                "RegOS could not locate any Village of Pinecrest Code of Ordinances sections relevant to this query. "
                "This typically means the topic falls outside the scope of Village of Pinecrest Code of Ordinances, "
                "or the question may need more specific regulatory terminology."
            )
            next_steps = (
                "Try rephrasing your question with specific Village of Pinecrest Code of Ordinances section references "
            ...
            next_steps = (
                ...
                "or 'system override'). RegOS is designed to answer Village of Pinecrest Code of Ordinances compliance questions."
            )
            ...
            body = (
                ...
                if self.valves.neo4j_failover_message
                    ...
                    "Village of Pinecrest Code knowledge graph database. Do not rely on this response for "
            ...
            next_steps = (
                "Try rephrasing your question to focus on Village of Pinecrest Code of Ordinances compliance requirements."
            )
...
and cross-referenced sections in the Village of Pinecrest municipal code knowledge graph.
            ...
            for i in range(len(messages) - 1, -1, -1):
                if messages[i].get("role") == "user":
                    messages[i]["content"] = (
                        ...
                        "assistant for Village of Pinecrest Code of Ordinances. I can help with "
        ...
        if self.valves.neo4j_failover_enabled:
            ...
            if not neo4j_ok:
                ...
                failover_system = (
                    ...
                    "You are responding WITHOUT any Village of Pinecrest Code of Ordinances regulatory context. "
