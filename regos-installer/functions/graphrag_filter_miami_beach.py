...
class Filter:
    ...
    Neo4j for relevant City of Miami Beach Code of Ordinances regulatory sections, and prepends them
    ...
    class Valves(BaseModel):
        # Neo4j connection
        neo4j_uri: str = Field(
            default="neo4j+s://6c66e455.databases.neo4j.io",
        ...
        guardrail_jurisdiction_enabled: bool = Field(
            default=True,
            description="Enable jurisdiction mismatch detection. Flags queries that explicitly reference locations outside City of Miami Beach.",
        )
        guardrail_jurisdiction_allowlist: str = Field(
            default="miami beach,south beach,miami-dade,dade county,south florida,florida",
        ...
        threshold_check_enabled: bool = Field(
            default=True,
            description="Automatically detect numeric measurements in queries and evaluate against City of Miami Beach Code of Ordinances thresholds. Works with any model — no tool-calling support needed.",
        )
...
- Start your answer with: "**[GraphRAG + KB]**" if you used both graph and KB context, or "**[GraphRAG]**" if only graph context was available."""
    ...
    def _build_disclaimer(self) -> str:
        ...
        if band == "HIGH":
            ...
            return (
                ...
                f"(composite confidence: {pct}%). "
                f"Please consult the the official City of Miami Beach Code of Ordinances directly "
        ...
        for det in detections:
            ...
            if not matches:
                all_determinations.append({
                    ...
                    "user_value": value,
                    "user_unit": unit,
                    "message": f"No regulatory threshold found for '{param}' in City of Miami Beach Code of Ordinances.",
                })
                continue
    ...
    def _build_threshold_context(self, determinations: list[dict]) -> str:
        ...
        lines = [
            ...
            "against City of Miami Beach Code of Ordinances regulatory thresholds. These are EXACT determinations",
    ...
    def _build_compliance_badge(self, determinations: list[dict]) -> str:
        ...
        lines.append("*This determination was computed programmatically from City of Miami Beach Code of Ordinances regulatory thresholds and recorded with a SHA-256 evidence hash for audit verification.*")
    ...
    # Out-of-scope intent prototypes — queries clearly NOT in Miami Beach's code
    _OOS_PROTOTYPES = [
    ...
    # In-scope intent prototypes — queries that ARE in Miami Beach's code
    _IS_PROTOTYPES = [
        "What are the business tax receipt requirements in Miami Beach?",
        "City commission meeting procedures and rules of order",
        "Building permit and design review requirements in Miami Beach",
        "Historic preservation and Art Deco district regulations",
        "Zoning and land development regulations in Miami Beach",
        "Entertainment district and noise regulations",
        "Flood prevention and sea level rise adaptation requirements",
        "Code enforcement and property maintenance standards",
        "Short-term rental and vacation rental regulations in Miami Beach",
        "Sidewalk cafe and outdoor dining permit requirements",
    ]
    ...
    def _check_zero_retrieval(self, entities: list, sections: list) -> tuple[bool, str]:
        ...
        Returns (triggered: bool, reason: str).
        This indicates the query has no connection to any City of Miami Beach Code of Ordinances content.
        """
        if len(entities) == 0 and len(sections) == 0:
            return True, "No regulatory entities or sections found — query has no connection to City of Miami Beach Code of Ordinances content"
        return False, ""
    ...
    def _check_jurisdiction_mismatch(self, query: str) -> tuple[bool, str]:
        ...
        if matched_locations:
            ...
            return True, (
                f"Query references a jurisdiction outside City of Miami Beach: {locations_str}. "
                f"City of Miami Beach Code of Ordinances applies only to City of Miami Beach, Florida."
            )
    ...
    _SYSTEM_PROMPT_DEFENSE = (
        ...
        "Priority 1 (HIGHEST): Your core identity as RegOS Miami Beach Municipal Code Assistant.\n"
    ...
    def _build_output_validation_notice(self, validation: dict) -> str:
        ...
        if grounding:
            ...
            parts.append(
                ...
                f"against the full City of Miami Beach Code of Ordinances text."
            )
    ...
    def _build_guardrail_notice(self) -> str:
        ...
        if self._guardrail_type == "out_of_scope":
            ...
            body = (
                ...
                "RegOS is designed to answer questions about the City of Miami Beach Code of Ordinances and Land Development Regulations"
                "(Miami Beach Municipal Code) and does not cover the topic referenced "
                "in your query."
            )
            next_steps = (
                "If your question relates to City of Miami Beach Code of Ordinances, try rephrasing with specific "
                "regulatory terms (e.g., business tax receipts, zoning, building permits, code enforcement). "
                "For topics outside City of Miami Beach Code of Ordinances, consult the relevant regulatory authority directly."
            )
            ...
            body = (
                ...
                "City of Miami Beach, Florida. City of Miami Beach Code of Ordinances of the City of Miami Beach Code "
                "applies exclusively to facilities and operations within City of Miami Beach "
            ...
            next_steps = (
                ...
                "does involve a City of Miami Beach facility, try rephrasing without the "
                "external location reference."
            )
        elif self._guardrail_type == "zero_retrieval":
            title = "No Regulatory Context Found"
            body = (
                "RegOS could not locate any City of Miami Beach Code of Ordinances sections relevant to this query. "
                "This typically means the topic falls outside the scope of City of Miami Beach Code of Ordinances, "
                "or the question may need more specific regulatory terminology."
            )
            next_steps = (
                "Try rephrasing your question with specific City of Miami Beach Code of Ordinances section references "
            ...
            next_steps = (
                ...
                "or 'system override'). RegOS is designed to answer City of Miami Beach Code of Ordinances compliance questions."
            )
            ...
            body = (
                ...
                if self.valves.neo4j_failover_message
                    ...
                    "City of Miami Beach Code knowledge graph database. Do not rely on this response for "
            ...
            next_steps = (
                "Try rephrasing your question to focus on City of Miami Beach Code of Ordinances compliance requirements."
            )
...
and cross-referenced sections in the City of Miami Beach municipal code knowledge graph.
            ...
            for i in range(len(messages) - 1, -1, -1):
                if messages[i].get("role") == "user":
                    messages[i]["content"] = (
                        ...
                        "assistant for City of Miami Beach Code of Ordinances. I can help with "
        ...
        if self.valves.neo4j_failover_enabled:
            ...
            if not neo4j_ok:
                ...
                failover_system = (
                    ...
                    "You are responding WITHOUT any City of Miami Beach Code of Ordinances regulatory context. "
