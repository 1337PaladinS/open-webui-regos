...
class Filter:
    ...
    Neo4j for relevant City of North Miami Beach Code of Ordinances regulatory sections, and prepends them
    ...
    class Valves(BaseModel):
        # Neo4j connection
        neo4j_uri: str = Field(
            default="neo4j+s://09231fed.databases.neo4j.io",
        ...
        max_section_chars: int = Field(
            default=8000,
            description="Maximum characters per section (truncates long sections). Set high to respect chunking pipeline's own sizing — 95th percentile is ~7000 chars.",
        )
        ...
        guardrail_jurisdiction_enabled: bool = Field(
            default=True,
            description="Enable jurisdiction mismatch detection. Flags queries that explicitly reference locations outside City of North Miami Beach.",
        )
        guardrail_jurisdiction_allowlist: str = Field(
            default="north miami beach,nmb,miami-dade,dade county,south florida,florida",
        ...
        threshold_check_enabled: bool = Field(
            default=True,
            description="Automatically detect numeric measurements in queries and evaluate against City of North Miami Beach Code of Ordinances thresholds. Works with any model — no tool-calling support needed.",
        )
    ...
    def _search_documents(self, query: str) -> list[dict]:
        """Fulltext search on Chunk nodes (text + breadcrumb) via chunk_search index."""
        driver = self._get_driver()
        ...
        try:
            with driver.session(database=self.valves.neo4j_database) as session:
                result = session.run(
                    """
                    CALL db.index.fulltext.queryNodes('chunk_search', $search_term)
                    YIELD node, score
                    WHERE score >= $min_score
                    RETURN node.chunk_id AS id,
                           node.section_title AS title,
                           node.text AS content,
                           node.breadcrumb AS breadcrumb,
    ...
    def _search_entities_by_name(self, query: str) -> list[dict]:
        """Search Role nodes by matching query terms against role names.
        ...
        try:
            with driver.session(database=self.valves.neo4j_database) as session:
                result = session.run(
                    """
                    MATCH (ro:Role)
                    WHERE any(term IN $terms WHERE toLower(ro.name) CONTAINS term)
                    RETURN ro.name AS name, 'role' AS type, elementId(ro) AS eid
                    LIMIT $limit
                    """,
                    terms=terms[:6],
                    limit=self.valves.entity_search_limit,
    ...
    def _get_sections_for_matched_entities(self, entity_eids: list[str]) -> list[dict]:
        """Traverse MENTIONS_ROLE from Chunk to Role to find chunks mentioning matched roles."""
        if not entity_eids:
            return []
        driver = self._get_driver()
        try:
            with driver.session(database=self.valves.neo4j_database) as session:
                result = session.run(
                    """
                    MATCH (c:Chunk)-[:MENTIONS_ROLE]->(ro:Role)
                    WHERE elementId(ro) IN $eids
                    WITH c, collect(DISTINCT ro.name) AS matched_entities, count(DISTINCT ro) AS entity_count
                    RETURN c.chunk_id AS id,
                           c.text AS content,
                           c.section_title AS title,
    ...
    def _get_sections_via_concepts(self, section_ids: list[str]) -> list[dict]:
        """From matched chunks, expand through REFERENCES relationships to find related chunks."""
        if not section_ids:
            return []
        driver = self._get_driver()
        try:
            with driver.session(database=self.valves.neo4j_database) as session:
                result = session.run(
                    """
                    MATCH (c:Chunk)-[:REFERENCES]->(related:Chunk)
                    WHERE c.chunk_id IN $section_ids
                      AND NOT related.chunk_id IN $section_ids
                    RETURN DISTINCT related.chunk_id AS id,
                           related.text AS content,
                           related.section_title AS title,
                           related.breadcrumb AS breadcrumb,
                           related.section_number AS section_number,
                           c.section_title AS concept,
                           1.0 AS score
                    LIMIT $limit
                    """,
    ...
    def _search_sections_direct(self, query: str) -> list[dict]:
        """Direct fulltext search on Chunk content via chunk_search index."""
        driver = self._get_driver()
        safe_query = self._escape_lucene(query)
        try:
            with driver.session(database=self.valves.neo4j_database) as session:
...
- Start your answer with: "**[GraphRAG + KB]**" if you used both graph and KB context, or "**[GraphRAG]**" if only graph context was available."""
    ...
    def _build_disclaimer(self) -> str:
        ...
        if band == "HIGH":
            ...
            return (
                ...
                f"(composite confidence: {pct}%). "
                f"Please consult the the official City of North Miami Beach Code of Ordinances directly "
        ...
        for det in detections:
            ...
            if not matches:
                all_determinations.append({
                    ...
                    "user_value": value,
                    "user_unit": unit,
                    "message": f"No regulatory threshold found for '{param}' in City of North Miami Beach Code of Ordinances.",
                })
                continue
    ...
    def _build_threshold_context(self, determinations: list[dict]) -> str:
        ...
        lines = [
            ...
            "against City of North Miami Beach Code of Ordinances regulatory thresholds. These are EXACT determinations",
    ...
    def _build_compliance_badge(self, determinations: list[dict]) -> str:
        ...
        lines.append("*This determination was computed programmatically from City of North Miami Beach Code of Ordinances regulatory thresholds and recorded with a SHA-256 evidence hash for audit verification.*")
    ...
    # Out-of-scope intent prototypes — queries clearly NOT in North Miami Beach's code
    _OOS_PROTOTYPES = [
        "What are the EPA federal discharge regulations?",
        "OSHA workplace safety requirements for my facility",
        "What are the IRS tax code requirements for businesses?",
        "Immigration law requirements and visa applications",
        "Criminal law penalties for environmental crimes",
        "Federal highway regulations and DOT compliance",
        "FDA food safety inspection requirements",
        "SEC financial reporting requirements",
        "Medicare and Medicaid billing compliance",
        "HIPAA patient privacy requirements",
    ]

    # In-scope intent prototypes — queries that ARE in North Miami Beach's code
    _IS_PROTOTYPES = [
        "What are the business tax receipt requirements in North Miami Beach?",
        "City commission meeting procedures and rules of order",
        "Building permit and inspection requirements in North Miami Beach",
        "Code of ethics and conflict of interest rules for city officials",
        "Zoning and land development regulations in North Miami Beach",
        "Public utilities and water service regulations",
        "Fire prevention and emergency management requirements",
        "Code enforcement and property maintenance standards",
        "Parks and recreation facility regulations in North Miami Beach",
        "Commercial signage and advertising regulations",
    ]
    ...
    def _check_zero_retrieval(self, entities: list, sections: list) -> tuple[bool, str]:
        ...
        Returns (triggered: bool, reason: str).
        This indicates the query has no connection to any City of North Miami Beach Code of Ordinances content.
        """
        if len(entities) == 0 and len(sections) == 0:
            return True, "No regulatory entities or sections found — query has no connection to City of North Miami Beach Code of Ordinances content"
        return False, ""
    ...
    def _check_jurisdiction_mismatch(self, query: str) -> tuple[bool, str]:
        ...
        if matched_locations:
            ...
            return True, (
                f"Query references a jurisdiction outside City of North Miami Beach: {locations_str}. "
                f"City of North Miami Beach Code of Ordinances applies only to City of North Miami Beach, Florida."
            )
    ...
    _SYSTEM_PROMPT_DEFENSE = (
        ...
        "Priority 1 (HIGHEST): Your core identity as RegOS North Miami Beach Municipal Code Assistant.\n"
    ...
    def _build_output_validation_notice(self, validation: dict) -> str:
        ...
        if grounding:
            ...
            parts.append(
                ...
                f"against the full City of North Miami Beach Code of Ordinances text."
            )
    ...
    def _build_guardrail_notice(self) -> str:
        ...
        if self._guardrail_type == "out_of_scope":
            ...
            body = (
                ...
                "RegOS is designed to answer questions about the City of North Miami Beach Code of Ordinances and Land Development Regulations"
                "(North Miami Beach Municipal Code) and does not cover the topic referenced "
                "in your query."
            )
            next_steps = (
                "If your question relates to City of North Miami Beach Code of Ordinances, try rephrasing with specific "
                "regulatory terms (e.g., business tax receipts, zoning, building permits, code enforcement). "
                "For topics outside City of North Miami Beach Code of Ordinances, consult the relevant regulatory authority directly."
            )
            ...
            body = (
                ...
                "City of North Miami Beach, Florida. City of North Miami Beach Code of Ordinances of the City of North Miami Beach Code "
                "applies exclusively to facilities and operations within City of North Miami Beach "
            ...
            next_steps = (
                ...
                "does involve a City of North Miami Beach facility, try rephrasing without the "
                "external location reference."
            )
        elif self._guardrail_type == "zero_retrieval":
            title = "No Regulatory Context Found"
            body = (
                "RegOS could not locate any City of North Miami Beach Code of Ordinances sections relevant to this query. "
                "This typically means the topic falls outside the scope of City of North Miami Beach Code of Ordinances, "
                "or the question may need more specific regulatory terminology."
            )
            next_steps = (
                "Try rephrasing your question with specific City of North Miami Beach Code of Ordinances section references "
                "or regulatory terms such as business tax receipts, building permits, zoning, "
                "or code enforcement procedures."
            )
            ...
            next_steps = (
                ...
                "or 'system override'). RegOS is designed to answer City of North Miami Beach Code of Ordinances compliance questions."
            )
            ...
            body = (
                ...
                if self.valves.neo4j_failover_message
                    ...
                    "City of North Miami Beach Code knowledge graph database. Do not rely on this response for "
            ...
            next_steps = (
                "Try rephrasing your question to focus on City of North Miami Beach Code of Ordinances compliance requirements."
            )
...
by matching your question against Chunk nodes, Role entities,
and cross-referenced sections in the City of North Miami Beach municipal code knowledge graph.
...
{citation_refs}"""
    ...
    async def inlet(
            ...
            for i in range(len(messages) - 1, -1, -1):
                if messages[i].get("role") == "user":
                    messages[i]["content"] = (
                        ...
                        "assistant for City of North Miami Beach Code of Ordinances. I can help with "
        ...
        if self.valves.neo4j_failover_enabled:
            ...
            if not neo4j_ok:
                ...
                failover_system = (
                    ...
                    "You are responding WITHOUT any City of North Miami Beach Code of Ordinances regulatory context. "
        ...
        try:
            ...
            # Step 1: Fulltext search on Chunk nodes (text + breadcrumb)
            ...
            if self.valves.show_trace:
                ...
                trace_lines.append("## Step 1: Document Fulltext Search (Chunk nodes via chunk_search)")
                ...
                trace_lines.append("## Step 2a: Entity Traversal (Role → MENTIONS_ROLE → Chunk)")
                ...
                trace_lines.append("## Step 2b: Cross-Reference Expansion (Chunk → REFERENCES → Chunk)")
                ...
                trace_lines.append("## Step 3: Direct Fulltext Search (Chunk nodes via chunk_search)")
