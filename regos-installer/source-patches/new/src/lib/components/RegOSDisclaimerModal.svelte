<script lang="ts">
	...
	export let disclaimerConfig: { title?: string; body?: string; accept_label?: string } | null = null;

	$: title = disclaimerConfig?.title || 'RegOS Compliance Copilot';
	$: acceptLabel = disclaimerConfig?.accept_label || 'I Understand, Let\'s Go';
	$: customBody = disclaimerConfig?.body || '';
	$: hasCustomBody = customBody.trim().length > 0;
...
<Modal bind:show size="md">
	...
	<div class="w-full p-4 px-6 text-gray-700 dark:text-gray-100">
		<div class="space-y-4 text-sm leading-relaxed">
			{#if hasCustomBody}
				<!-- Admin-configured disclaimer body (markdown rendered) -->
				<div class="prose dark:prose-invert prose-sm max-w-none">
					{@html renderMarkdown(customBody)}
				</div>
			{:else}
				<!-- Default hardcoded disclaimer -->
				<p>
		...
		<div class="flex justify-end pt-4 text-sm font-medium">
			...
			>
				<span class="relative">{acceptLabel}</span>
			</button>
		</div>
	</div>
</Modal>
