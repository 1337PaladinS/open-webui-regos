...
# ── 4. Create/Update RegOS Model with System Prompt ──
echo ""
echo "  4. Creating RegOS model with system prompt..."

SYSTEM_PROMPT=$(python3 -c "
import json
with open('${SCRIPT_DIR}/../system_prompt.md') as f:
    print(json.dumps(f.read()))
")
...
echo "       → Toggle 'Global' ON if not already"
echo "    3. Upload your Knowledge Base documents"
echo "       → Admin > Knowledge > Create Collection"
echo "       → Upload Chapter 24 PDF/DOCX files"
echo "    4. Select 'RegOS Compliance Copilot' as your model in the chat"
echo ""
echo "  Test it:"
